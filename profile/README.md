# CONFLICT

**Bespoke Technology Atelier**

We design and build production-grade software for teams that won't settle for average.
Precision execution. No bloat. No drama.

→ [weareconflict.com](https://weareconflict.com)

---

## Open Source

### [ScuttleBot](https://github.com/ConflictHQ/scuttlebot) — [scuttlebot.dev](https://scuttlebot.dev)
Run a fleet of AI agents. Watch them work. Talk to them directly. Coordination backplane for Claude, Codex, and Gemini — each appears as a named IRC user in a shared channel, observable in real time from any IRC client.

### [Navegador](https://github.com/ConflictHQ/navegador) — [navegador.dev](https://navegador.dev)
AST + knowledge graph context engine for AI coding agents. Parses your codebase into a queryable property graph and layers your team's knowledge on top — decisions, concepts, rules, ownership. Ships as a CLI, Python SDK, and MCP server.

### [BoilerWorks](https://github.com/ConflictHQ/boilerworks) — [boilerworks.dev](https://boilerworks.dev/) - [boilerworks.ai](https://boilerworks.ai)
Opinionated, AI-native project templates for shipping production software fast. Django + Next.js full-stack boilerplate with auth, GraphQL, Docker, and infrastructure wired end-to-end.

### [PlanOpticon](https://github.com/ConflictHQ/PlanOpticon) — [planopticon.dev](https://planopticon.dev)
AI-powered video analysis and knowledge extraction. Goes through a video and captures notes, diagrams, and structured knowledge from the conversation.

### [Hugoifier](https://github.com/ConflictHQ/hugoifier) — [hugoifier.dev](https://hugoifier.dev)
Converts HTML/JS/CSS templates and themes into Hugo themes, wired up to Decap CMS.

---

## What We Do

- Platform & infrastructure engineering
- AI/ML systems and data pipelines
- Web, mobile, and API development
- eCommerce architecture
- DevOps, SRE, and on-call support
- MVPs, internal tools, and innovation sprints

---

## Work With Us

We're selective. If you need elite engineering with speed and taste:

→ [hello@weareconflict.com](mailto:hello@weareconflict.com)

---

*No open roles for now.*
